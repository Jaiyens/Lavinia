# SA 92896223 — metadata-only dossier

> Source: `normalized/by-meter/92896223.json` (primary), `normalized/inventory.json` row 134, `normalized/manifest.json` idx 133.
...
| Descriptor | none |
| PG&E Service Agreement (SA) ID | **92896223** |
| Meter serial # | **1009545848** |
...
| Latitude / Longitude | **36.257485, -120.103739** |
| Inventory row | 134 |
...
1. **Get the bill.** Either the **account bill PDF for 0096005793-3** (AI vision extraction in `src/lib/extract` turns the PDF into the JSON the levers read) **or a UtilityAPI pull** for SA **92896223**. Either one populates `billing`, `summaries`, and (if solar) `nem`.
...
> "SA 92896223 runs on **HAGA2** under account 0096005793-3, but we hold **only inventory metadata for it** — no bill, usage, or solar settlement — so there are no dollar findings yet. Pull the 0096005793-3 bill PDF or a UtilityAPI feed for SA 92896223 and the levers can run."
