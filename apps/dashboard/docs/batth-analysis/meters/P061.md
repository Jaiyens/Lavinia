# P061 (PUMP #60) — ELKHORN SHOP — metadata-only dossier

> Source: `normalized/by-meter/5090079335.json` (primary), `normalized/inventory.json` row 16, `normalized/manifest.json` idx 15.
...
| Grower pump id | **P061** |
| Descriptor | PUMP #60 |
| PG&E Service Agreement (SA) ID | **5090079335** |
| Meter serial # | **1010127902** |
...
| Latitude / Longitude | **36.492473, -119.894833** |
| Inventory row | 16 |

This SA is a 900 GPM pump, on the ELKHORN SHOP ranch. Billing entity **KANWARJIT BATTH & GAGANDIP BATTH** on account **5090219363-5**.
...
- **Array / group:** Solar group label **4444** (a grouping/array tag, not a per-meter nameplate). Per-meter kW is null on this row; true array size lives on the farm-level solar inventory (the two real arrays total 1,932 kW).
...
1. **Get the bill.** Either the **account bill PDF for 5090219363-5** (AI vision extraction in `src/lib/extract` turns the PDF into the JSON the levers read) **or a UtilityAPI pull** for SA **5090079335**. Either one populates `billing`, `summaries`, and (if solar) `nem`.
...
> "P061 (ELKHORN SHOP) runs on **AG5B** under account 5090219363-5, but we hold **only inventory metadata for it** — no bill, usage, or solar settlement — so there are no dollar findings yet. Pull the 5090219363-5 bill PDF or a UtilityAPI feed for SA 5090079335 (it is a legacy AG-5 meter worth a rate review), plus the solar Generation Allocation Summary, and the levers can run."
