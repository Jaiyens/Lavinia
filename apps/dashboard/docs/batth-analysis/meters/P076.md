# P076 — PUMP 76 (KAMM CORNELIA) — metadata-only meter dossier

> Source: `normalized/inventory.json` rowNumber 32, `normalized/manifest.json` (Excel inventory).
...
| Inventory rowNumber | 32 |
| Stored rate schedule | `AG5B` |

This is a known, good-condition irrigation pump on the **KAMM CORNELIA** ranch, serving **ALMONDS**, rated at **1,400 GPM**.
...
> P076 (KAMM CORNELIA, almonds) is on the old AG-5 rate with solar, but it sits on a **different PG&E account that we don't have a bill for** — so we can't show real numbers yet. Pull this meter's bill (or a 15-minute meter read) and we can check whether the old rate is costing you money and settle the solar true-up.
