// One-time seed: build the committed representative-farm fixture from REAL Fresno County parcels
// around the canonical test point, auto-enriched from free public layers. Run:
//   npx tsx scripts/seed-representative-farm.ts
// Writes apps/dashboard/fixtures/representative-farm.json (real geometry + baked enrichment only;
// the operational model is regenerated at render time by representative.ts). Re-run to refresh.
...
import type { FixtureParcel, RepresentativeFarmFixture } from "../src/lib/parcel/farm/seed";
...
const CENTER = { lat: 36.6004616, lng: -119.7817871 };
const HALF = 0.02; // ~2 km half-box, to have enough candidates to cluster from
...
async function main(): Promise<void> {
  ...
  console.log("Fetching Fresno parcels in the seed envelope...");
  ...
  const fixture: RepresentativeFarmFixture = {
    generatedAt: new Date().toISOString(),
    name: "Cordova Ranches",
    county: "Fresno",
    center: CENTER,
    parcels,
  };
  const out = join(process.cwd(), "fixtures", "representative-farm.json");
