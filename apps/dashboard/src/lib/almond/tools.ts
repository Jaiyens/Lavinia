 ...
 * receives these, so the model can never call them). As of Story 8.5 this is `exportSpreadsheet`:
 * it WRITES a file (a real capability beyond the read-safe public set), so it is owner-only.
 *
 * `toModelOutput` collapses the tool's result for the MODEL's context to a tiny text summary, so the
 * spreadsheet bytes never enter the prompt window (they are lifted onto the UI stream by the
 * responder instead). The full result, including the bytes, is still available to the responder via
 * `onStepFinish`'s tool results - that is where the download card is written.
 */
